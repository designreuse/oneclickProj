...
<div class="content">
	...
	<p class="taC pageGuide mt40">회원탈퇴가 정상적으로 이루어졌습니다.</p>
	<c:choose>
		<c:when test="${('1' eq selectMenu) && ('40' eq custStat)}">
			<p class="taC pageGuide2 mt10">이랜드리테일 원클릭 제휴사이트의 서비스는 기존대로 이용하실 수 있습니다.<br />다른 제휴사이트의 회원탈퇴를 원하실 경우 각 사이트에서 회원탈퇴를 신청해주시기 바랍니다.<br />재가입을 원하실 경우 이랜드리테일 원클릭에서 간단한 동의절차 후 가입이 가능합니다. </p>
		</c:when>
		<c:when test="${('1' eq selectMenu) && ('50' eq custStat)}">
			<p class="taC pageGuide2 mt10">회원정보는 30일간 보관되며, 30일 이내에 고객센터를 통해 탈퇴 철회하실 수 있습니다.<br />30일 이후, 회원정보는 삭제되며, 원상복구가 불가합니다. </p>
		</c:when>
	...
	<div class="btnDiv taC mt35">
		<a href="javascript:locateKap();" class="btn blue large w305"><span id="countDown">5초후에 E:LAND MALL(으)로 이동합니다</span></a>
	</div>
