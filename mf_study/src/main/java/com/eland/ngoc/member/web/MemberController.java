...
public class MemberController {
    ...
    public String getJoinMemberStep3(HttpServletRequest request
		...
		try {
			...
			model.addAttribute("siteCode", CommonUtil.getSessionSiteCode(request));
    ...
    public String getFindId(HttpServletRequest request, Model model) {
		...
		String siteCode = request.getParameter("siteCode");
		String scIdReturnUrl = request.getParameter("returnUrl");
		...
		if (StringUtil.isNotEmpty(scIdReturnUrl)) {
			
			if (StringUtil.isEmpty((String)request.getSession().getAttribute("scIdReturnUrl"))) {
				request.getSession().setAttribute("scIdReturnUrl", scIdReturnUrl);
			}
		}
		model.addAttribute("scIdReturnUrl", scIdReturnUrl);
		model.addAttribute("isMember", request.getParameter("isMember"));
    ...
    public String getFindPwd(HttpServletRequest request, Model model) {
		...
		String siteCode = request.getParameter("siteCode");
		String scPwdReturnUrl = request.getParameter("returnUrl");
		...
		if (StringUtil.isNotEmpty(scPwdReturnUrl)) {
			
			if (StringUtil.isEmpty((String)request.getSession().getAttribute("scIdReturnUrl"))) {
				request.getSession().setAttribute("scPwdReturnUrl", scPwdReturnUrl);
			}
		}
		model.addAttribute("scPwdReturnUrl", scPwdReturnUrl);
		model.addAttribute("isMember", request.getParameter("isMember"));
	...
	public String idScResultPopup(Model model, HttpServletRequest request) throws Exception{
		...
		String[] reqNum = CommonUtil.getCookieReqNum(request);
		logger.debug("####reqNum1 = " + reqNum[0]);
		logger.debug("####reqNum2 = " + reqNum[1]);
		String siteCode = CommonUtil.getSessionSiteCode(request);
		AuthDtls authDtls = new AuthDtls();
		Pcc pcc;
		Ipin ipin;
		...
		model.addAttribute("scIdReturnUrl", (String)request.getSession().getAttribute("scIdReturnUrl"));
	...
	public String pwdScResultPopup(Model model, HttpServletRequest request) throws Exception{
		...
		model.addAttribute("scPwdReturnUrl", (String)request.getSession().getAttribute("scPwdReturnUrl"));
